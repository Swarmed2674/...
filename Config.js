...
module.exports = {
    ...
    */
    ...
    token: process.env.token || 'MTAxMTk5NDY2MTMwMjMyMTIyNA.GXyMYe.xFoClCgyT2uSt64fHVxd4mQPXWAqSBPjiMPx10',

    // 機器人註冊 [ / ] 斜線命令 會使用這個機器人ID (填入機器人ID 或者Developer Portal的Application ID，其實都一樣。)
    clientID: process.env.clientID || '1011994661302321224',
    clientSECRET: process.env.clientSECRET || '8jmc4oj0e_znSNLsouR_oduGoyMWSBq1',
    ...
    developers: process.env.developers || ['902523303795765248'],
    ...
    botPresence: {
        activities: [
            ...
            },
            {
                name: '訂閱 Kiawm91 ！',
                type: 'STREAMING',
                url: 'https://www.youtube.com/channel/UCv6CAdzvJX7LBH8Fx6qEvqQ',
            },
            {
                name: 'Kiawm91 訂閱數：{Youtube.subs}位！',
                type: 'COMPETING',
            },
        ],
        // 狀態
    ...
    invite_code: process.env.invite_code || 'https://discord.gg/pkPEFvemQJ',

    /**
    * 機器人傳輸頻道設定
    */
    // 紀錄伺服器的ID
    ServerID: process.env.serverid || '1007565442463842335',
    // 紀錄伺服器的相關頻道
    Channels: {
        // 機器人所有記錄會在這個頻道 (填入ID)
        All: process.env.Channels_All || '',
        // 機器人啟動時會記錄在這個頻道 (填入ID)
        ClientOn: process.env.Channels_ClientOnChannel || '',
        // 若有錯誤的回報(各種)會記錄在這個頻道 (填入ID)
        report: process.env.Channels_reportChannel || '',
        // 機器人指令的使用紀錄會記錄在這個頻道 (填入ID)
        commandRec: process.env.Channels_commandRecChannel || '',
        // 該伺服器的變動被記錄在這個頻道 (填入ID)
        serverRec: process.env.Channels_serverRecChannel || '',
        // 機器人被邀請進入伺服器的紀錄會記錄在這個頻道 (填入ID)
        inviteChannel: process.env.Channels_inviteChannel || '',
    },
    ...
    */
    // 網頁
    web: {
        noWeb: process.env.web_noWeb || null,
    ...
    */
    ...
    youtube: 'https://www.youtube.com/channel/UCv6CAdzvJX7LBH8Fx6qEvqQ',
    plugins: {
        // 抽獎系統
        giveaways: {
            // 是否要顯示創辦者?
    ...
    beta: {
        ...
        rpc: {
            ...
            setActivity: {
                ...
                smallImageText: 'SakuraShop 櫻花商店',
            },
        },
